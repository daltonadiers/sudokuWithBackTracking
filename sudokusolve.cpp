...
namespace sdk{
    class mySudo{
        public:
         void init(){
            std::cout << "Lendo arquivo e iniciando..." << std::endl << std::endl;
            chargeData();
         ...
         int sudoTable[9][9];
            void chargeData(){
                std::ifstream tabela("sudoku.txt"); int valor; int i=0; int j=0;
                while (tabela >> valor){
                    sudoTable[i][j] = valor;
                    j++;            
                    if(j==9){j=0;i++;}
            ...
            bool verifiy(int n, int i, int j){
                //FUNÇÕES QUE VERIFICAM SE UM NÚMERO PODE OU NÃO SER COLOCADO EM DETERMINADA POSIÇÃO
                if(!verifyLine(n,i))return false;
                if(!verifyColumn(n,j))return false;
                if(!verifyBlock(n,i,j))return false;
                if(!verifySet(n,i,j))return false;
                return true;
            }
            ...
            bool verifyLine(int n, int i){
                for(int j=0; j<9;j++){
                    if(sudoTable[i][j]==n)return false;
                }return true;
            }
            ...
            bool verifyBlock(int n, int i, int j){
                int minI,minJ;
                if(i<=2)minI=0;else if(i<=5)minI=3;else minI=6;
                if(j<=2)minJ=0;else if(j<=5)minJ=3;else minJ=6;
                for(int i_=minI; i_<minI+3; i_++){
                    for(int j_=minJ; j_<minJ+3; j_++){
                        if(sudoTable[i_][j_]==n)return false;
                    }
                }return true;
            }
            ...
            int changeNumber(int i, int j){
                for(int p=1; p<10;p++){
                    if(verifiy(p,i,j))return p;
                    else mapOfPairsToSets[std::make_pair(i, j)].insert(p);
                }
                return 20;
            }
    };
